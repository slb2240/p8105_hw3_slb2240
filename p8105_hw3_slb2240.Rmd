...
class(apples_coffee_dow$order_dow)
...
ny_noaa_dates = ny_noaa %>% 
  separate(date, c("year", "month", "day"), by = "/")
  View()
```
...
noaa_ny_jan_july = ny_noaa_dates %>% 
  ...
  filter(temp_max != "NA") %>% 
  select(id, month, temp_max) %>% 
  group_by(id, month) %>% 
  summarize(mean_temp = round(mean(as.numeric(temp_max)), digits = 2)) %>% 
  View()
  
ggplot(noaa_ny_jan_july, aes(x = id, y = mean_temp)) +
  geom_point(aes(color = month)) +
  geom_smooth(se = FALSE) +
  xlab("Weather Station ID") +
  ylab("Average Mean Temp, in tenths of degrees C")
